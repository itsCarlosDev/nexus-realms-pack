...
function nexusHordeDirectorForgetEntity(state, entityId, removeFromNativeTracking) {
...
function nexusHordeDirectorRemoveTrackedEntity(entity) {
  ...
  var directorOwnerId = nexusHordeDirectorEntityOwners.get(directorEntityId)
  if (!directorOwnerId) return

  var directorState = nexusHordeDirectorStates.get(directorOwnerId)
  if (!directorState) {
    nexusHordeDirectorEntityOwners.delete(directorEntityId)
    return
  }
  nexusHordeDirectorForgetEntity(directorState, directorEntityId, false)
}
...
function nexusHordeDirectorCleanupState(state) {
  ...
  directorEntityIds.forEach(directorEntityId => {
    nexusHordeDirectorForgetEntity(state, directorEntityId, false)
  })
  state.alive.clear()
...
function nexusHordeDirectorWaveAmount(state) {
  try {
    ...
    if (directorSpawnData) {
      return Math.max(1, Number(directorSpawnData.getSpawnAmount()) || 1)
    }
  } catch (error) {
    nexusHordeDirectorLogErrorOnce(
...
function nexusHordeDirectorLaunchWave(state) {
  ...
  state.zeroSince = -1
  state.nextWaveAt = -1
  state.settleAt = nexusHordeDirectorServerTick + NEXUS_HORDE_DIRECTOR_SPAWN_SETTLE_TICKS

  try {
    state.horde.spawnWave(state.player, nexusHordeDirectorWaveAmount(state))
  } catch (error) {
    state.currentWave -= 1
    state.phase = 'transition'
    state.nextWaveAt = nexusHordeDirectorServerTick + NEXUS_HORDE_DIRECTOR_INTERMISSION_TICKS
    nexusHordeDirectorLogErrorOnce(
...
function nexusHordeDirectorPresentWaveCleared(state) {
  try {
    if (typeof nexusHordePresentationMarkWaveCleared === 'function') {
...
function nexusHordeDirectorComplete(state) {
  ...
  try {
    if (typeof nexusHordePresentationShowVictory === 'function') {
      nexusHordePresentationShowVictory(state.player)
    }
    // false conserva la finalizacion funcional nativa y ejecuta una sola vez sus comandos/recompensas.
...
function nexusHordeDirectorRefreshTrackedState(state) {
  ...
  state.alive.forEach((directorRecord, directorEntityId) => {
    if (directorRecord.unloadedAt >= 0) {
      if (
        nexusHordeDirectorServerTick - directorRecord.unloadedAt >=
    ...
    try {
      if (!directorRecord.entity.isAlive()) directorExpiredIds.push(directorEntityId)
    } catch (ignored) {
      directorRecord.unloadedAt = nexusHordeDirectorServerTick
    }
  })

  directorExpiredIds.forEach(directorEntityId => {
    var directorRecord = state.alive.get(directorEntityId)
    var directorWasUnloaded = directorRecord && directorRecord.unloadedAt >= 0
    nexusHordeDirectorForgetEntity(state, directorEntityId, directorWasUnloaded)
...
function nexusHordeDirectorTickState(state) {
  ...
  if (state.phase === 'preparing') {
    if (nexusHordeDirectorServerTick >= state.firstWaveAt) {
  ...
  if (state.phase === 'transition') {
    if (nexusHordeDirectorServerTick < state.nextWaveAt) return
    if (state.currentWave >= NEXUS_HORDE_DIRECTOR_TOTAL_WAVES) {
  ...
  if (nexusHordeDirectorServerTick < state.settleAt || state.alive.size > 0) {
    state.zeroSince = -1
    return
  }

  if (state.zeroSince < 0) {
    state.zeroSince = nexusHordeDirectorServerTick
    return
  }
  if (
    nexusHordeDirectorServerTick - state.zeroSince <
...
ForgeEvents.onEvent('net.smileycorp.hordes.common.event.HordeStartEvent', event => {
  var directorStartPlayer = event.getPlayer()
  var directorStartPlayerId = nexusHordeDirectorPlayerId(directorStartPlayer)
  var directorPreviousState = nexusHordeDirectorStates.get(directorStartPlayerId)
  if (directorPreviousState) nexusHordeDirectorCleanupState(directorPreviousState)

  nexusHordeDirectorStates.set(
    directorStartPlayerId,
    nexusHordeDirectorCreateState(directorStartPlayer, event.getHorde())
  )
})

ForgeEvents.onEvent('net.smileycorp.hordes.common.event.HordeSpawnEntityEvent', event => {
  var directorSpawnPlayer = event.getPlayer()
  var directorSpawnState = nexusHordeDirectorStates.get(
    nexusHordeDirectorPlayerId(directorSpawnPlayer)
  )
  if (!directorSpawnState || directorSpawnState.phase !== 'wave') return

  var directorSpawnEntity = event.getEntity()
  var directorSpawnEntityId = nexusHordeDirectorEntityId(directorSpawnEntity)
  try {
    directorSpawnEntity.addTag(directorSpawnState.tag)
  } catch (error) {
    nexusHordeDirectorLogErrorOnce(
  ...
  directorSpawnState.alive.set(directorSpawnEntityId, {
    entity: directorSpawnEntity,
    unloadedAt: -1
  })
  nexusHordeDirectorEntityOwners.set(directorSpawnEntityId, directorSpawnState.playerId)
...
ForgeEvents.onEvent('net.minecraftforge.event.entity.living.LivingDeathEvent', event => {
  nexusHordeDirectorRemoveTrackedEntity(event.getEntity())
})

ForgeEvents.onEvent('net.minecraftforge.event.entity.EntityLeaveLevelEvent', event => {
  var directorLeavingEntity = event.getEntity()
  var directorLeavingEntityId = nexusHordeDirectorEntityId(directorLeavingEntity)
  var directorOwnerId = nexusHordeDirectorEntityOwners.get(directorLeavingEntityId)
  if (!directorOwnerId) return

  var directorLeavingState = nexusHordeDirectorStates.get(directorOwnerId)
  if (!directorLeavingState) return
  var directorLeavingRecord = directorLeavingState.alive.get(directorLeavingEntityId)
  if (!directorLeavingRecord) return

  try {
    if (!directorLeavingEntity.isAlive()) {
      nexusHordeDirectorForgetEntity(directorLeavingState, directorLeavingEntityId, false)
      return
    }
  } catch (ignored) {
    // Si no puede consultarse, se trata como descarga hasta que expire el fallback.
  }
  directorLeavingRecord.unloadedAt = nexusHordeDirectorServerTick
})

ForgeEvents.onEvent('net.minecraftforge.event.entity.EntityJoinLevelEvent', event => {
  var directorJoiningEntity = event.getEntity()
  var directorJoiningEntityId = nexusHordeDirectorEntityId(directorJoiningEntity)
  var directorOwnerId = nexusHordeDirectorEntityOwners.get(directorJoiningEntityId)
  if (!directorOwnerId) return

  var directorJoiningState = nexusHordeDirectorStates.get(directorOwnerId)
  if (!directorJoiningState) return
  var directorJoiningRecord = directorJoiningState.alive.get(directorJoiningEntityId)
  if (!directorJoiningRecord) return

  directorJoiningRecord.entity = directorJoiningEntity
  directorJoiningRecord.unloadedAt = -1
})

ForgeEvents.onEvent('net.smileycorp.hordes.common.event.HordeEndEvent', event => {
  var directorEndState = nexusHordeDirectorStates.get(
    nexusHordeDirectorPlayerId(event.getPlayer())
  )
  if (directorEndState) nexusHordeDirectorCleanupState(directorEndState)
})

ForgeEvents.onEvent('net.minecraftforge.event.entity.player.PlayerEvent$PlayerLoggedOutEvent', event => {
  var directorLogoutState = nexusHordeDirectorStates.get(
    nexusHordeDirectorPlayerId(event.getEntity())
  )
  if (directorLogoutState) directorLogoutState.paused = true
})

ForgeEvents.onEvent('net.minecraftforge.event.entity.player.PlayerEvent$PlayerLoggedInEvent', event => {
  var directorLoginPlayer = event.getEntity()
  var directorLoginState = nexusHordeDirectorStates.get(
    nexusHordeDirectorPlayerId(directorLoginPlayer)
  )
  if (directorLoginState) {
    directorLoginState.player = directorLoginPlayer
...
ForgeEvents.onEvent('net.minecraftforge.event.server.ServerStoppingEvent', event => {
  ...
  nexusHordeDirectorStates.forEach(directorState => directorStoppingStates.push(directorState))
  directorStoppingStates.forEach(directorState => nexusHordeDirectorCleanupState(directorState))
...
ForgeEvents.onEvent('net.minecraftforge.event.TickEvent$ServerTickEvent', event => {
  ...
  nexusHordeDirectorStates.forEach(directorState => {
    try {
      nexusHordeDirectorTickState(directorState)
    } catch (error) {
      nexusHordeDirectorLogErrorOnce(
